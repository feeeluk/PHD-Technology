<div id="BusinessAnalysisPortfolio" class="box--WithPadding item">

    <?php

        if (str_contains($_SERVER['REQUEST_URI'],'business_analysis') == true)
            {
                echo "<h1 class='item--Title'>PORTFOLIO</h1>";
                
                echo "<p class='item--CentreAlignedText'>Examples of work that I have done:</p>";
            }

        elseif (str_contains($_SERVER['REQUEST_URI'],'portfolio') == true)
            {             
                echo "<h2 class='item--Title'>BUSINESS ANALYSIS</h2>";     
            }

    ?>

    <div class="portfolio">

        <img src="/assets/images/portfolio/portfolio_placeholder.jpg" alt="Default Portfolio">
        
        <img src="/assets/images/portfolio/portfolio_placeholder.jpg" alt="Default Portfolio">

    </div>
        
</div>     
    
